...
;
    try {
        ...
        const mailOptions = {
            from: 'jordan.radanov94@gmail.com',
            to: email,
            subject: 'Welcome',
            text: 'Thank you for registering in the platform!'
        }

        transporter.sendMail(mailOptions, (error) => {
            if(error) {
                return res.status(500).json({status: false, message: "Email was not sent!"});
            }else {
                return res.status(200).json({status: true, message: "Email was sent succesfully!"});
            }
        })
    }
    catch(error) {
